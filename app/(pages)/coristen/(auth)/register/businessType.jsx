...
const BusinessType = ({ setStage, setStoreValues, values }) => {
  ...
  const OptionBox = ({ tabType, title, caption = "" }) => (
    <Box
      className={`w-44 flex-shrink-0 h-32 m-2 flex shadow flex-col items-center justify-center cursor-pointer translation-all duration-300 bg-white rounded-md border ${
        businessType !== tabType ? "border-white" : "border-blue-800"
    ...
    >
      ...
      <Box
        className={`h-5 w-5 mt-4 rounded-full border-4 ${
          businessType !== tabType ? "border-gray-200" : "border-blue-800"
