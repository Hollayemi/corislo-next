...
import { useGetMyOrdersQuery } from '@/app/redux/user/slices/orderSlice'
import { BasicModal } from '@/app/components/cards/popup'
import OrderActionConfirmation from './components'

const OrderPage = ({ searchParams }) => {
  const [orderShowing, setOrderShowing] = useState('All Orders')
  const [modalOpen, setModalOpen] = useState(false)
  const { page } = use(searchParams)
  const { data: result, refetch: refetchOrder } = useGetMyOrdersQuery({ status: orderShowing.split(' ')[0], page: page || 1 }, { skip: !orderShowing })
    ...
    <UserWrapper popup={
      ...
      <Box>
        <Box className="!px-2 my-2 sm:!px-16 md:!px-24 lg:!px-32 md:!py-7 relative">
          ...
          {/* timeline */}
          <Box className="hidden flexr items-center mt-6">
          ...
          <Box className="hidden flexr flex-col md:flex-row items-start md:h-68 mt-5 bg-blue-100">
          ...
          <Box className="hidden flexr items-center justify-between mt-8">
          ...
          <Box className=" flex items-center justify-between mt-8 relative">
              ...
              <Box>
                {orderArray.map((res, i) => (
                  <OrderProductView
                    ...
                    status={res?.currStatus}
                    orderSlug={res.orderSlug}
                    orderId={res._id}
                    createdAt={res.createdAt}
                    store={res.store}
                    popUpFunc={setModalOpen}
                    mutate={() => refetchOrder()}
                  />
                ))}
