...
const Policies = ({ store, branch }) => {
  ...
  const opening_hours = days.map(
    (x) =>
      ...
      `${formatName(x)}: from ${info.opening_hours[x].from} to ${info.opening_hours[x].to
      },`
  )
  return (
    <Box className="!bg-white rounded-xl !px-8 py-8 mt-10 !text-black">
      ...
      <EachExpression
        ...
        body={[
          `We offer ${info.pickup && info.waybill?.isset
            ? `both in-store pickup and waybill delivery options . For your convenience, you can pick up your order at the store or choose to have it delivered to your doorstep. Please note that a minimum purchase amount of ₦${info.waybill?.minimum_amount
            } is required for delivery, and waybill fees are paid  ${info.waybill?.waybill_fee_paid_seperately
              ? 'on delivery'
              : 'together with order.'
            }.`
            : info.pickup
              ...
              : `waybill delivery option . For your convenience, choose to have it delivered to your doorstep. Please note that a minimum purchase amount of ₦${info.waybill?.minimum_amount
              } is required for delivery, and waybill fees are paid ${info.waybill?.waybill_fee_paid_seperately
                ? 'on delivery'
                : 'together with order.'
              }.`
