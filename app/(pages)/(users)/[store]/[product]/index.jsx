...
import { useSaveItemMutation } from '@/app/redux/user/slices/saveItemSlice'
import { Loader } from 'lucide-react'

const ProductDisplay = ({ params: param, searchParams: searchParam }) => {
  const searchParams = use(searchParam)
  const dispatch = useDispatch()
  const { product: urlKey } = use(param)
  const { savedProds, refetchSavedItems } = useUserData()
  const { cartedProducts: cartedProds, addToCart, refetchCart } = useCart()
  ...
  return (
    <Box className="!px-2 sm:!px-16 md:!px-10 lg:!px-32 md:!py-7">
      <Box className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {/* Product Images */}
        <Box className="gir mt-4 md:mt-0">
              ...
              <Box className="flex items-center">
                ...
                <Box
                  ...
                  onClick={() => saveItem({
                    ...
                    branch: product?.branch,
                    quantity: quantity,
                  }).then((res) => res.type === "success" && refetchSavedItems())}
                >
                  {saving ? <Loader className="animate-spin" size={15} /> : <IconifyIcon
                    icon="tabler:heart"
                    className={`!text-xl !mr-1 text-[24px] ${savedProds.includes(product?._id) ? '!text-red-500' : ''
                      }`}
                  />}
                  <Typography
                    variant="body2"
                    className={`!text-sm !mr-1 ${savedProds.includes(product?._id) ? '!text-red-500' : ''
                      }`}
                  >
              ...
              <Box className="pt-4 border-t border-gray-200">
                ...
                <Box className="flex flex-col sm:flex-row gap-3">
                  <Button
                    ...
                    onClick={() => addToCart(payload).then((res) => res.type === "success" && refetchCart())}
