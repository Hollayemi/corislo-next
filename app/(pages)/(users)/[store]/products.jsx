...
const StoreProducts = ({ store, branch, searchParams }) => {
  const { data, isLoading } = useSWR(
    `/store/products-campaign?store=${store}&branch=${branch}&type=discounted&page=${searchParams?.dp || 1
    }`
  )
  const result = data ? data.data : {}
  const { data: myProduct, isLoading: otherLoading } = useSWR(
    `/store/products-campaign?store=${store}&branch=${branch}&page=${searchParams?.p || 1
    }`
  )
    ...
    <Box className="!bg-white rounded-xl !px-3 py-5 mt-10">
      ...
      {result?.products?.all?.length &&
        <>
          ...
          </Box><Box className="flex justify-center mt-6">
            <MyPagination
