...
const CategoryPage = ({ params }) => {
  ...
  const { data: getCategory } = useSWR(`/corisio/all-categories?id=${theCategory}`)
  const category = getCategory?.data || {}
  const [group, setGroup] = useState([])
  const { data: getData, error } = useSWR(`/corisio/all-sub-categories?category=${theCategory}`)
  ...
  return (
    <UserWrapper className=" !px-2 md:!px-8">
