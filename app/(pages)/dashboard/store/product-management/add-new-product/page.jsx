...
import useProductForm from '@/app/hooks/useUploadProductForm'
import { useGetOneProductsQuery } from '@/app/redux/business/slices/productSlice'
...
const AddNewProduct = ({ params: param, searchParams }) => {
    ...
    const { edit: editID } = use(searchParams)
    console.log(editID)
    const brk = theme.breakpoints.down


    const { data: toEdit, isLoading: gettingToEdit } = useGetOneProductsQuery({ id: editID }, { skip: !editID });
    const { data: cates, isLoading: cateLoading } = useGetFeaturedCategoriesQuery("true")

    const categories = cates ? cates?.data : []
    const prodToEdit = toEdit ? toEdit.data : {}

    const dataToEdit = toEdit ? { data: prodToEdit } : undefined
    ...
    const isloading = cateLoading || gettingToEdit || loading
    ...
    return (
        ...
        >
            <Box className="bg-white rounded-md px-3 md:!px-5 pt-6 pb-8 !text-[13px]">
                    ...
                    <Grid item xs={12} md={5}>
                        ...
                        <Box sx={{ pl: 0.2, pl: brk('md') && '0.5', mb: 0.5 }}>
                            ...
                            <TextField
                                ...
                                id="outlined-basic"
                                onChange={(e) => handleTextChange('prodName', e.target.value)}
                            />
                        </Box>
                        ...
                        <Box sx={{ pl: 0.2, pl: brk('md') && '0.5', mb: 1.5 }}>
                            ...
                            <SimpleDropDown
                                ...
                                onChange={(e) => handleSubCateSelection(e.target.value)}
                            ...
                            <SimpleDropDown
                                ...
                                onChange={(e) => handleProductGroupSelection(e.target.value)}
                                label="Product Class"
                                sx={{ mb: 2 }}
                            />
                        </Box>
                        ...
                        <Box sx={{ pl: 0.2, pl: brk('md') && '0.5', mb: 2.5 }}>
                            <TextField
                                sx={{ mb: 0.5 }}
                                onChange={(e) => handleNumericChange('prodPrice', e.target.value)}
                                type="number"
                        ...
                        <Box sx={{ pl: 0.2, pl: brk('md') && '0.5', mb: 2.5 }}>
                            <TextField
                                sx={{ mb: 0.5 }}
                                onChange={(e) => handleNumericChange('totInStock', e.target.value)}
                                type="number"
                                fullWidth
                    ...
                    <Grid item xs={12} md={7}>
                        ...
                        <Box className="pl-2 md:pl-4 mb-5">
                            ...
                            <div className="flex items-center flex-wrap">
                                {formData?.images?.map((each, i) => (
                                    <div className="relative w-16 h-16 m-3" key={i}>
                                        <img
                            ...
                            <>
                                ...
                                <Box className="pl-2 md:pl-4 mb-4">
                                    <SizeComponent
                                        ...
                                        setSelectedSizes={handleSizeSelection}
                                    />
                                </Box>
                        ...
                        {specInfo && (
                            <Box className="mt-4">
                                <Box className="flex justify-center !px-2 !mt-3 w-full">
                                    <Box className="flex flex-col w-full">
                                        <Autocomplete
                                            ...
                                            onChange={(e, newValue) => setNewSpecKey(newValue)}
                                            value={newSpecKey}
                                            onInputChange={(e, newValue) =>
                                                newValue && setNewSpecKey(newValue)
                                            }
                                            renderInput={(params) => (
                                                <TextField
                                                    {...params}
                                        ...
                                        <Box className="flex items-center w-full">
                                            <Autocomplete
                                                options={
                                                    specInfo?.spec?.[newSpecKey?.replaceAll(' ', '_')] || []
                                                }
                                ...
                                {Object.entries(formData.specifications).length > 0 && (
                                    <Box className="border border-gray-200 rounded-lg my-5 p-3">
                                        {Object.entries(formData.specifications?.variations || {}).map(([key, value], i) => (
                                            <Box
                                                key={key}
                                                className={`flex cursor-pointer hover:bg-gray-50 justify-between !h-6 items-center ${i > 0 && "mt-4"} p-2  last:border-b-0 `}
                                            >
                                                <Typography onClick={() => { setNewSpecKey(key); setSpecValue(value) }} className={`font-medium text-base !text-gray-700`}>
                                                    {key}: {value}

                                                </Typography>
                                                <Box onClick={() => removeSpec(key)}>
                                                    <Trash
                                                        size={18}
                                                        color="#EF4444"
                                                    />
                        ...
                        <TextField
                            ...
                            onChange={(e) => handleTextChange('prodInfo', e.target.value)}
                            maxRows={7}
