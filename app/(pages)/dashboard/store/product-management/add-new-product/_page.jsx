...
const AddNewProduct = ({ params: param, searchParams }) => {
  ...
  useEffect(() => {
    console.log({ categories, cate: formData.category })
    setFromCollection(() => categories.filter((x) => x._id === formData.category)[0] || null);
  }, [formData.category, prodToEdit, categories]);

  const isloading = cateLoading || prodLoading || loading
  const reset = () => {
    setFormData({
      prodName: '',
      prodPrice: '',
    ...
    if (toEdit) {
      ...
      setFormData(() => {
        return {
          ...
          specifications: toEditData.specifications || {},
          category: toEditData.categoryId,
          _id: toEditData._id,
        }
      })
      selectDelivery(toEditData.delivery || ["pickup"]);
      setProdSpecs(toEditData.specifications || {});
      fromCollection?.sub_category?.map((sub) => sub._id === toEditData.subcategory && setGroups(sub.groups || []));
      productGroups.map((group) => group._id === toEditData.productGroup && setProdSpecs(group.spec || ""));
    }
  }, [toEdit, formData.category, prodToEdit, categories, fromCollection?.sub_category, productGroups])

  console.log({ productGroups, fromCollection })

  console.log({ totl: formData.specifications })
              ...
              <div className="flex items-center flex-wrap">
                {formData?.images?.map((each, i) => (
                  <div className="relative w-16 h-16 m-3" key={i}>
                    ...
                    />
                    <div
                      onClick={() => setFormData((prev) => ({ ...prev, images: formData?.images?.filter(e => e !== each) }))}
                      className="text-[6px] flex items-center justify-center text-white absolute -mt-2 -mr-2 top-0 right-0 w-4 h-4 rounded-full bg-red-500"
                    >
                      <X fontSize={12} />
                    </div>
                ...
                {/* {Object.entries(formData.specifications).length > 0 && (
                  <Box className="border border-gray-200 rounded-lg my-5 p-3">
                    {Object.entries(formData.specifications).map(([key, value], i) => (
                      ...
                      >
                        <Typography onClick={() => { setNewSpec(key); setSpecValue(value) }} className={`font-medium text-base !text-gray-700`}>
                          {key}: {value}
                        </Typography>
