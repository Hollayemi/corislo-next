...
const ChatApp = () => {
    ...
    useEffect(() => {
        ...
        if (newUpdates?.log) {
            // If we receive the full log array, use it
            setMessageLog(prev => ({
                ...prev,
                log: newUpdates.log
            }));
    ...
    const contacts = stores?.map((each) => ({
        ...
        phone: each.customer?.phone || each.branchDetails?.phone,
        username: mySubstring(each.customer?.fullname, 17),
        myPic: each.customer?.picture || '/images/default-avatar.png',
        role: 'customer',
        log: each.log,
        avatar: each.customer?.picture || '/images/default-avatar.png',
        branchId: each.branchDetails?.branchId,
        lastMessage: each.lastMessage?.message || '',
        time: timeSince(each.lastMessage?.time),
        unread: each.userUnread || 0,
        online: true
    })) || [];
                ...
                {opened.id ? (
                                    ...
                                    return (
                                        <div key={msg.id} className={`flex ${isCustomer ? 'justify-start' : 'justify-end'}`}>
                                            <div className="flex items-end space-x-2 max-w-xs lg:max-w-md">
                                                ...
                                                {!isCustomer && (
                                                    <img
                                                        src={selectedContact.myPic || '/images/default-avatar.png'}
                                                        alt="Store"
                                                        className="w-8 h-8 rounded-full object-cover"
                                                    />
                                                )}
                    ...
                    <div className="flex w-full flex-col items-center justify-center">
                        <Image
                            ...
                            className="w-32 h-32"
                        />
                        <h4 className="mt-4 text-gray-600">Select a customer to chat with</h4>
                    </div>
                )}
            </div>
        </StoreLeftSideBar>
