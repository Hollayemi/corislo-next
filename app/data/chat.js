...
mock.onGet('/apps/chat/chats-and-contacts').reply(() => {
  const chatsContacts = chatData.chats.map(chat => {
    const contact = chatData.contacts.find(c => c.id === chat.userId)
  ...
  const contactsToShow = chatData.contacts.filter(co => {
    return !chatData.chats.some(ch => {
      return co.id === ch.id
    })
  })

  const profileUserData = {
    id: chatData.profileUser.id,
    avatar: chatData.profileUser.avatar,
    fullName: chatData.profileUser.fullName,
    status: chatData.profileUser.status
  }
...
// ------------------------------------------------
mock.onGet('/apps/chat/users/profile-user').reply(() => [200, chatData.profileUser])
...
mock.onGet('/apps/chat/get-chat').reply(config => {
  ...
  const chat = chatData.chats.find(c => c.id === userId)
  if (chat) chat.unseenMsgs = 0
  const contact = chatData.contacts.find(c => c.id === userId)
...
mock.onPost('/apps/chat/send-msg').reply(config => {
  ...
  let activeChat = chatData.chats.find(chat => chat.id === obj.contact.id)
  ...
  if (activeChat === undefined) {
    ...
    activeChat = chatData.chats[chatData.chats.length - 1]
  } else {
  ...
  reorderChats(
    chatData.chats,
    chatData.chats.findIndex(i => i.id === response.id),
    0
  )
